...
const UserButton: React.FC = () => {
  const [info, setInfo] = useState<DocumentData | null | undefined >(null);
  const authCtx = useContext(AuthContext);
  const userRef: DocumentReference<DocumentData> = doc(db, "users", authCtx.id);

  
  if (info === null) {
    getDoc(userRef).then((doc: DocumentSnapshot<DocumentData>) => {
      if (!doc.exists) {
